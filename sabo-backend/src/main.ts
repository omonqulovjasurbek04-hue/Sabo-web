...
async function bootstrap() {
  ...
  const origins = configService.get<string[]>('cors.origins', ['http://localhost:3000']);
  ...
  if (nodeEnv !== 'production' || process.env.ENABLE_SWAGGER === 'true') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .addTag('Certificates', 'Official ISO / HACCP / Halal Verification Documents')
  ...
  logger.log(`🚀 SABO Backend API running on http://localhost:${port}/${apiPrefix}`);
}
