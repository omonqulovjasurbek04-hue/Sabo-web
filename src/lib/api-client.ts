...
export const apiClient = {
  ...
  getProducts: (params?: { category?: string; q?: string; page?: number; limit?: number }) => {
    ...
    return request<any[]>(`/api/v1/products${qs ? `?${qs}` : ""}`);
  },
  getProductBySlug: (slug: string) => request<any>(`/api/v1/products/${slug}`),
  ...
  getMe: () => request<{ user: any }>("/api/v1/auth/me"),
  createOrder: (payload: {
    ...
    request<{ order: any; paymentUrl?: string }>("/api/v1/orders", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
